import { Request, Response } from 'express';
import { Op } from 'sequelize';
import db from '../models/index';
import { IEmprestimo, ILivro, IMulta } from '../models/types/types';

const Emprestimo = db.Emprestimo;
const Livro = db.Livro;
const Multa = db.Multa;
const Usuario = db.Usuario;

export const registrarDevolucao = async (req: Request, res: Response) => {
  try {
    const { emprestimoId } = req.body;

    if (!emprestimoId) {
      return res.status(400).send({ message: 'O ID do empréstimo é obrigatório' });
    }

    const emprestimo = await Emprestimo.findByPk(emprestimoId) as IEmprestimo | null;  

    if (!emprestimo) {
      return res.status(404).send({ message: 'Empréstimo não encontrado' });
    }

    if (emprestimo.status === 'devolvido') {
      return res.status(400).send({ message: 'Livro já foi devolvido' });
    }
    if (emprestimo.status === 'atrasado') {
      return res.status(400).send({ message: 'Livro já foi devolvido com atraso' });
    }

    const dataAtual = new Date();
    const dataPrevista = new Date(emprestimo.dataPrevistaDevolucao);
    const diasAtraso = Math.max(0, Math.floor((dataAtual.getTime() - dataPrevista.getTime()) / (1000 * 60 * 60 * 24)));

    emprestimo.dataDevolucao = dataAtual;
    emprestimo.diasAtraso = diasAtraso;
    emprestimo.status = diasAtraso > 0 ? 'atrasado' : 'devolvido';
    await emprestimo.save?.();  

    let multa = null;
    if (diasAtraso > 0) {
      const valorMulta = diasAtraso * 2;

      multa = await Multa.create({
        emprestimoId: emprestimo.id,
        usuarioId: emprestimo.usuarioId,
        valorMulta: valorMulta,
        status: 'pendente'
      }) as unknown as IMulta;  
    }

    const livro = await Livro.findByPk(emprestimo.livroId) as ILivro | null;  

    if (livro) {
      livro.quantidadeDisponivel += 1;
      await livro.save?.();  
    }

    res.status(200).send({
      message: 'Devolução registrada com sucesso',
      diasAtraso: emprestimo.diasAtraso,
      multaAplicada: diasAtraso > 0,
      valorMulta: multa ? multa.valorMulta : 0
    });
  } catch (error) {
    console.error('Erro ao registrar devolução:', error); 
    res.status(500).send({ message: 'Erro ao registrar devolução' }); 
  }
};
export const listarDevolucoes = async (req: Request, res: Response) => {
  try {
    const devolucoes = await Emprestimo.findAll({
      where: {
        status: {
          [Op.in]: ['devolvido', 'atrasado']
        }
      },
      include: [
        { model: Usuario, attributes: ['id', 'nome', 'email'] },
        { model: Livro, attributes: ['id', 'titulo', 'autor'] }
      ],
      order: [['dataDevolucao', 'DESC']]
    });

    res.status(200).send(devolucoes);
  } catch (error) {
    console.error('Erro ao listar devoluções:', error);
    res.status(500).send({ message: 'Erro ao listar devoluções' });
  }
};

